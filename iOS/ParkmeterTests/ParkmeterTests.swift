import XCTest
@testable import Parkmeter

@MainActor
final class ParkmeterTests: XCTestCase {
    var store: ParkmeterStore!

    override func setUp() async throws {
        store = ParkmeterStore()
    }
    ...
    func testFreeLimitBlocksAddition() {
        store.isPro = false
        for i in 0..<(ParkmeterStore.freeLimit + 5) {
            store.add(title: "Filler \(i)", amount: 1.0)
        }
        XCTAssertEqual(store.entries.count, ParkmeterStore.freeLimit)
    }
    ...
    func testCanAddMoreWhenPro() {
        ...
        XCTAssertGreaterThan(store.entries.count, ParkmeterStore.freeLimit)
    }
    ...
    func testFreeLimitAboveSeedCount() {
        XCTAssertGreaterThan(ParkmeterStore.freeLimit, 3)
    }
}
